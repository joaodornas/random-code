...
load LSD_aal.mat; %cell with nsubjectsXconditions, in each cell a matrix with nregionsxtimepoints
...
for seed=1:N
 ...
 mev(seed)=mean(squeeze(max(squeeze(IntegStim(seed,:,1:nevents(seed))),[],1)));
 stdev(seed)=std(squeeze(max(squeeze(IntegStim(seed,:,1:nevents(seed))),[],1)));
%  mevokedQ(seed,:)=mean(squeeze(IntegStimQ(seed,:,1:nevents(seed))),2);
end

save (sprintf('4TR_Ignition_CASE%d.mat', CASE), 'mev', 'stdev', 'mevokedinteg');
