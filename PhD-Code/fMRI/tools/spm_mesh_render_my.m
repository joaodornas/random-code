function varargout = spm_mesh_render_my(action,varargin)
% Display a surface mesh & various utilities
% FORMAT H = spm_mesh_render_my('Disp',M,'PropertyName',propertyvalue)
...
% handle.
%
% FORMAT H = spm_mesh_render_my(M)
% Shortcut to previous call format.
%
% FORMAT H = spm_mesh_render_my('ContextMenu',AX)
% AX       - axis handle or structure returned by spm_mesh_render_my('Disp',...)
%
% FORMAT H = spm_mesh_render_my('Overlay',AX,P)
% AX       - axis handle or structure given by spm_mesh_render_my('Disp',...)
% P        - data to be overlayed on mesh (see spm_mesh_project)
%
% FORMAT H = spm_mesh_render_my('ColourBar',AX,MODE)
% AX       - axis handle or structure returned by spm_mesh_render_my('Disp',...)
% MODE     - {['on'],'off'}
%
% FORMAT H = spm_mesh_render_my('ColourMap',AX,MAP)
% AX       - axis handle or structure returned by spm_mesh_render_my('Disp',...)
% MAP      - a colour map matrix
%
% FORMAT MAP = spm_mesh_render_my('ColourMap',AX)
% Retrieves the current colourmap.
%
% FORMAT spm_mesh_render_my('Register',AX,hReg)
% AX       - axis handle or structure returned by spm_mesh_render_my('Disp',...)
...
% Guillaume Flandin
% $Id: spm_mesh_render_my.m 5109 2012-12-11 20:53:42Z guillaume $
...
switch lower(action)
    ...
    case 'disp'
        ...
        %-Add context menu
        %------------------------------------------------------------------
        spm_mesh_render_my('ContextMenu',H);
    ...
    case {'colourbar', 'colorbar'}
        ...
        if size(d,1) > 1
            ...
            set(H.colourbar,'YTick',1:size(d,1));
        else
            set(ic,'CData',c);
            ...
            set(H.colourbar,'YTick',clim(2):((clim(3)-clim(2))/4):clim(3));
    ...
    case 'snapshot'
        ...
        H = getHandles(varargin{1});
        snapshot_label = varargin{2};
        
        H.rotate3d = rotate3d(H.axis);
        set(H.rotate3d,'Enable','on');
        
        pathname = 'K:\Dropbox (Uni Magdeburg)\';
        
        disp('Go to Y-Z view (right)');
            myRotate(H, [90 0]);
            snapshot_filename = strcat(snapshot_label,'-','right');
            myOwnSave(H,snapshot_filename,pathname);
        disp('Go to Y-Z view (left)')
            myRotate(H, [-90 0]);
            snapshot_filename = strcat(snapshot_label,'-','left');
            myOwnSave(H,snapshot_filename,pathname);
        disp('Go to X-Y view (top)');
            myRotate(H, [0 90]);
            snapshot_filename = strcat(snapshot_label,'-','top');
            myOwnSave(H,snapshot_filename,pathname);
        disp('Go to X-Y view (bottom)');
            myRotate(H, [-180 -90]);
            snapshot_filename = strcat(snapshot_label,'-','bottom');
            myOwnSave(H,snapshot_filename,pathname);
        disp('Go to X-Z view (front)')
            myRotate(H, [-180 0]);
            snapshot_filename = strcat(snapshot_label,'-','front');
            myOwnSave(H,snapshot_filename,pathname);
        disp('Go to X-Z view (back)');
            myRotate(H, [0 0]);
            snapshot_filename = strcat(snapshot_label,'-','back');
            myOwnSave(H,snapshot_filename,pathname);
    ...
    otherwise
        try
            H = spm_mesh_render_my('Disp',action,varargin{:});
        catch
...
spm_mesh_render_my('Colourbar',H,toggle(get(obj,'Checked')));

%==========================================================================
function myColourmap(obj,evt,H)
spm_mesh_render_my('Colourmap',H,feval(get(obj,'Label'),256));
...
%==========================================================================
function myOwnSave(H,filename,pathname)
...
[min_value, max_value] = getGlobalMinMax;

cb = colorbar();

c(1:size(col,1),1,1:size(col,2)) = col;
ic = findobj(cb,'Type','image');
if size(d,1) > 1
    set(ic,'CData',c(1:size(d,1),:,:));
    set(ic,'YData',[1 size(d,1)]);
    set(cb,'YLim',[1 size(d,1)]);
    set(cb,'YTickLabel',[]);
    set(cb,'YTick',1:size(d,1));
else
    set(ic,'CData',c);
    clim = getappdata(H.patch,'clim');
    %if isempty(clim), clim = [false min(d) max(d)]; end
    if isempty(clim), clim = [false min_value max_value]; end
    set(ic,'YData',clim(2:3));
    set(cb,'YLim',clim(2:3));
    set(cb,'YTick',clim(2):((clim(3)-clim(2))/4):clim(3));
end

if isdeployed
    %deployprint(h, '-dpng', '-opengl', fullfile(pathname, filename));
    %deployprint(h, '-depsc', '-opengl', fullfile(pathname, filename));
    deployprint(h, '-dpng', '-opengl', strcat(filename,'.png'));
    deployprint(h, '-depsc', '-opengl', strcat(filename,'.eps'));
else
    %print(h, '-dpng', '-opengl', fullfile(pathname, filename));
    %print(h, '-depsc', '-opengl', fullfile(pathname, filename));
    print(h, '-dpng', '-opengl', strcat(filename,'.png'));
    print(h, '-depsc', '-opengl', strcat(filename,'.eps'));
end
close(h);
...
if ~sts, return; end
spm_mesh_render_my('Overlay',H,P);
...
%--------------------------------------------------------------------------
if isfield(H,'colourbar')
    spm_mesh_render_my('Colourbar',H);
end
