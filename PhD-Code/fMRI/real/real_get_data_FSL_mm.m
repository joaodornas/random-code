...
function [output_data, mask_data, settings] = real_get_data_FSL_mm(settings,kind,run,file,mask,get_at_this_preprocessed_step,mm)
...
switch idx_kind
    ...
    case 2
        ...
        filename = strcat(settings.passive.FSL.run(run).fullpath,'\',file,'-',int2str(mm),'mm','.nii');
    ...
    case 3
        ...
        filename = strcat(settings.trials.FSL.run(run).fullpath,'\',file,'-',int2str(mm),'mm','.nii');
    ...
    case 4
        ...
        filename = strcat(settings.restingstate.FSL.run(run).fullpath,'\',file,'-',int2str(mm),'mm','.nii');
