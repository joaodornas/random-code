function real_FC_voxel_AAL_ROI_kmeans_v2
...
% getCorrelationsOfMeansInsideClustersV2;

% doFCOnAllClustersV2;

% plotFCOnAllClustersMeanRunsV2;

% doContrastsFCOnAllClustersV2;

% plotContrastFCOnAllClustersV2;

% computeGrangerAmongClustersV2;

% getClusterParcellationInfoAndMeanTimeSeriesV2;

% save3DVolumeContrastFCOnAllClustersPOSNEGV2;

% computeGrangerAtGroupLevelV2;

% plotFCAndGrangerAtGroupLevelContrastJBV2;

% getMeaningfulClustersWithChangeBasedOnFCGCAttentionStimulusOnlyV2;

% plotFCAndGrangerAtGroupLevelContrastJBV2;

% parcellation_label = {'MD758','SP758','MD758-NoWavelet'};
% for parcels=parcellation_label
%     plotFCOnAllClustersMeanRunsV2(parcels{1});
% end

% parcellation_label = {'MD758','SP758','MD758-NoWavelet'};
% for parcels=parcellation_label
%     plotGrangerAtGroupLevelContrastJBAllLobesV2(parcels{1});
% end
...
% for parcels=parcellation_label
%     for does=do
%         plotFCAndGrangerAtGroupLevelContrastJBV2(parcels{1},does{1});
%     end
% end
...
% do = 'GC';
% parcellation_label = 'MD758-beforeCorrected';
% plotFCAndGrangerAtGroupLevelContrastJBV2(parcellation_label,do);
...
function getClusterParcellationInfoAndMeanTimeSeriesV2
...
% nROI = 90;
% for iROI=1:nROI
%     ROI(iROI).label = AAL_ROI(iROI).Nom_L;
%     ROI(iROI).nClusters = length(ROI_clusters(iROI).clusters);
%     for iCluster=1:ROI(iROI).nClusters
%         ROI(iROI).clusters(iCluster).idx_voxels = ROI_clusters(iROI).clusters(iCluster).idx_voxels;
%     end
% end
...
for iSet=1:nSet
    ...
    %% file = settings.FSL.files.functional.custom.residual_voxel; %% DO AFTER ALL PREPROCESS STEPS
    %% file = settings.FSL.files.functional.custom.residual_voxel_WITHOUT_Wavelet; %% DO WITH ALL PREPROCESS STEPS EXCEPT WAVELET
    file = settings.FSL.files.functional.custom.wavelet_voxel; %% DO WITH ALL PREPROCESS STEPS EXCEPT LR
...
save(strcat('FC-Voxels-',parcellation_label,'-AAL-ROI-corr-KMeans-Info-Mean-TS-corrected-NoLR.mat'),'ROI');
...
label = 'SP758-v2';
load('Anatomical-Spatial-Clusters-v2a.mat');
...
fname = strcat(label,'-For-Plot','.nii');
...
function getCorrelationsOfMeansInsideClustersV2

load('FC-Voxels-AAL-ROI-corr-KMeans-Info-Mean-TS-corrected.mat');
...
save('FC-Voxels-AAL-ROI-corr-KMeans-FC-Inside-Clusters-corrected.mat','FC_means_clusters');
...
function doFCOnAllClustersV2

% load('FC-Voxels-AAL-ROI-corr-KMeans-Info-Mean-TS-corrected.mat'); %%% ORIGINAL WITH CORRECTION FROM THE BUG
% load('FC-Voxels-MD758-AAL-ROI-corr-KMeans-Info-Mean-TS-corrected-NoWavelet.mat'); %%% WITHOUT WAVELET
load('FC-Voxels-MD758-AAL-ROI-corr-KMeans-Info-Mean-TS-corrected-NoLR.mat'); %%% WITHOUT LR
...
% save('FC-Voxels-MD758-AAL-ROI-corr-KMeans-FC-All-Clusters-corrected.mat','FC_clusters');
% save('FC-Voxels-MD758-AAL-ROI-corr-KMeans-FC-All-Clusters-corrected-NoWavelet.mat','FC_clusters');
save('FC-Voxels-MD758-AAL-ROI-corr-KMeans-FC-All-Clusters-corrected-NoLR.mat','FC_clusters');

end

function plotFCOnAllClustersMeanRunsV2(parcellation_label)

nRun = 32;

switch parcellation_label
    
    case 'MD758'
        
        % parcellation_label = 'MD758';
        load('FC-Voxels-AAL-ROI-corr-KMeans-FC-All-Clusters-corrected.mat'); %%% MD 758
        load('FC-Voxels-AAL-ROI-corr-KMeans-Info.mat'); %%% MD 758
        
    case 'SP758'

        % parcellation_label = 'SP758';
        load('Anatomical-Spatial-Clusters-v2-Corr.mat'); %%% SPATIAL 758
        load('Anatomical-Spatial-Clusters-v2.mat'); %%% SPATIAL 758

        % ROI = ROI_clusters;

        for iRun=1:nRun
            FC_clusters(iRun).track_rho = Spatial_Clusters(iRun).track_corr;
            FC_clusters(iRun).passive_rho = Spatial_Clusters(iRun).passive_corr;
            FC_clusters(iRun).rest_rho = Spatial_Clusters(iRun).rest_corr;
        end
        
    case 'MD758-NoWavelet'
        
        % parcellation_label = 'MD758';
        load('FC-Voxels-MD758-AAL-ROI-corr-KMeans-FC-All-Clusters-corrected-NoWavelet.mat'); %%% MD 758 - NoWavelet
        load('FC-Voxels-AAL-ROI-corr-KMeans-Info.mat'); %%% MD 758
...
for iRun=1:nRun
    ...
    all_corr_track(iRun,:,:) = FC_clusters(iRun).track_rho;
    all_corr_passive(iRun,:,:) = FC_clusters(iRun).passive_rho;
    all_corr_rest(iRun,:,:) = FC_clusters(iRun).rest_rho;
    
end

%%% DOING FISHER TRANSFORM %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

all_corr_track_z = (1/2).*log((1 + all_corr_track)./(1 - all_corr_track));
all_corr_passive_z = (1/2).*log((1 + all_corr_passive)./(1 - all_corr_passive));
all_corr_rest_z = (1/2).*log((1 + all_corr_rest)./(1 - all_corr_rest));
            
my_std_track_z = squeeze(std(all_corr_track_z,0,1));
my_mean_track_z = squeeze(mean(all_corr_track_z,1));

my_std_passive_z = squeeze(std(all_corr_passive_z,0,1));
my_mean_passive_z = squeeze(mean(all_corr_passive_z,1));

my_std_rest_z = squeeze(std(all_corr_rest_z,0,1));
my_mean_rest_z = squeeze(mean(all_corr_rest_z,1));

new_label = strcat('FC-',parcellation_label,'-Track-Fisher-checker');
plotFCcheckerboard( my_mean_track_z, my_std_track_z, cmin, cmax, log_mu_threshold, applyThreshold, applyColorBar, new_label );
% plotFCgeneral(my_mean_track_z,my_std_track_z,new_label,plotParcelsLabels_idx,ROI);
% new_label = strcat('Mean-STD-',parcellation_label,'-Track-Fisher');
% MyContourCorrelation_v5(my_mean_track_z(:),my_std_track_z(:),new_label,1);

new_label = strcat('FC-',parcellation_label,'-Passive-Fisher-checker');
plotFCcheckerboard( my_mean_passive_z, my_std_passive_z, cmin, cmax, log_mu_threshold, applyThreshold, applyColorBar, new_label );
% plotFCgeneral(my_mean_passive_z,my_std_passive_z,new_label,plotParcelsLabels_idx,ROI);
% new_label = strcat('Mean-STD-',parcellation_label,'-Passive-Fisher');
% MyContourCorrelation_v5(my_mean_passive_z(:),my_std_passive_z(:),new_label,1);

new_label = strcat('FC-',parcellation_label,'-Rest-Fisher-checker');
plotFCcheckerboard( my_mean_rest_z, my_std_rest_z, cmin, cmax, log_mu_threshold, applyThreshold, applyColorBar, new_label );
% plotFCgeneral(my_mean_rest_z,my_std_rest_z,new_label,plotParcelsLabels_idx,ROI);
% new_label = strcat('Mean-STD-',parcellation_label,'-Rest-Fisher');
% MyContourCorrelation_v5(my_mean_rest_z(:),my_std_rest_z(:),new_label,1);

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

% %%% DOING ERROR FUNCTION TO GET P-VALUE %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% 
% % my_std_track = squeeze(std(all_corr_track,0,1));
% my_mean_track = squeeze(mean(all_corr_track,1));
% 
% % my_std_passive = squeeze(std(all_corr_passive,0,1));
% my_mean_passive = squeeze(mean(all_corr_passive,1));
% 
% % my_std_rest = squeeze(std(all_corr_rest,0,1));
% my_mean_rest = squeeze(mean(all_corr_rest,1));
% 
% mean_track_pval = erfc((abs(my_mean_track).*sqrt(nTR))./sqrt(2));
% mean_passive_pval = erfc((abs(my_mean_passive).*sqrt(nTR))./sqrt(2));
% mean_rest_pval = erfc((abs(my_mean_rest).*sqrt(nTR))./sqrt(2));
% 
% my_mean_track(mean_track_pval > pcriterion) = 0;
% my_mean_passive(mean_passive_pval > pcriterion) = 0;
% my_mean_rest(mean_rest_pval > pcriterion) = 0;
% 
% label = strcat('FC-',parcellation_label,'-Track-ERFC');
% plotFCgeneralERFC(my_mean_track,label,plotParcelsLabels_idx);
% 
% label = strcat('FC-',parcellation_label,'-Passive-ERFC');
% plotFCgeneralERFC(my_mean_passive,label,plotParcelsLabels_idx);
% 
% label = strcat('FC-',parcellation_label,'-Rest-ERFC');
% plotFCgeneralERFC(my_mean_rest,label,plotParcelsLabels_idx);
...
function doContrastsFCOnAllClustersV2

nRuns = 32;

% parcellation_label = 'MD758';
% load('FC-Voxels-AAL-ROI-corr-KMeans-FC-All-Clusters-corrected.mat');
% load('FC-Voxels-AAL-ROI-corr-KMeans-Info.mat');
...
parcellation_label = 'MD758-NoWavelet';
load('FC-Voxels-MD758-AAL-ROI-corr-KMeans-FC-All-Clusters-corrected-NoWavelet.mat');
load('FC-Voxels-AAL-ROI-corr-KMeans-Info.mat');
...
save(strcat('FC-Voxels-',parcellation_label,'-AAL-ROI-corr-KMeans-FC-All-Clusters-Contrast-corrected.mat'),'contrast_stimulus_z','contrast_attention_z');

end

function doContrastsFCOnAllClustersPOSNEGV2
...
for iRun=1:nRuns
    ...
    track_corr(iRun,:,:) = FC_clusters(iRun).track_rho;
    passive_corr(iRun,:,:) = FC_clusters(iRun).passive_rho;
    rest_corr(iRun,:,:) = FC_clusters(iRun).rest_rho;
    
end

dens_track_pos = zeros(nRuns,nClusters);
dens_passive_pos = zeros(nRuns,nClusters);
dens_rest_pos = zeros(nRuns,nClusters);

for iRun=1:nRuns
    
    for iCluster=1:nClusters
        
        line = squeeze(track_corr(iRun,iCluster,:));
        dens_track_pos(iRun,iCluster) = length(find(line > 0))/nClusters;
        dens_track_neg(iRun,iCluster) = length(find(line < 0))/nClusters;
        
        line = squeeze(passive_corr(iRun,iCluster,:));
        dens_passive_pos(iRun,iCluster) = length(find(line > 0))/nClusters;
        dens_passive_neg(iRun,iCluster) = length(find(line < 0))/nClusters;
        
        line = squeeze(rest_corr(iRun,iCluster,:));
        dens_rest_pos(iRun,iCluster) = length(find(line > 0))/nClusters;
        dens_rest_neg(iRun,iCluster) = length(find(line < 0))/nClusters;
        
    end
    
end
   
p_attention_pos = zeros(1,nClusters);
h_attention_pos = zeros(1,nClusters);
p_stimulus_pos = zeros(1,nClusters);
h_stimulus_pos = zeros(1,nClusters);

p_attention_neg = zeros(1,nClusters);
h_attention_neg = zeros(1,nClusters);
p_stimulus_neg = zeros(1,nClusters);
h_stimulus_neg = zeros(1,nClusters);

for iCluster=1:nClusters
    
   if mod(iCluster,10) == 0; disp(int2str(iCluster)); end
    
   p_attention_pos(iCluster) = ranksum(squeeze(dens_track_pos(:,iCluster)),squeeze(dens_passive_pos(:,iCluster)));
   
   if median(squeeze(dens_track_pos(:,iCluster))) > median(squeeze(dens_passive_pos(:,iCluster))); h_attention_pos(iCluster) = -1; end
   if median(squeeze(dens_track_pos(:,iCluster))) < median(squeeze(dens_passive_pos(:,iCluster))); h_attention_pos(iCluster) = 1; end
    
   p_stimulus_pos(iCluster) = ranksum(squeeze(dens_passive_pos(:,iCluster)),squeeze(dens_rest_pos(:,iCluster)));
   
   if median(squeeze(dens_passive_pos(:,iCluster))) > median(squeeze(dens_rest_pos(:,iCluster))); h_stimulus_pos(iCluster) = -1; end
   if median(squeeze(dens_passive_pos(:,iCluster))) < median(squeeze(dens_rest_pos(:,iCluster))); h_stimulus_pos(iCluster) = 1; end

   p_attention_neg(iCluster) = ranksum(squeeze(dens_track_neg(:,iCluster)),squeeze(dens_passive_neg(:,iCluster)));
   
   if median(squeeze(dens_track_neg(:,iCluster))) > median(squeeze(dens_passive_neg(:,iCluster))); h_attention_neg(iCluster) = -1; end
   if median(squeeze(dens_track_neg(:,iCluster))) < median(squeeze(dens_passive_neg(:,iCluster))); h_attention_neg(iCluster) = 1; end
    
   p_stimulus_neg(iCluster) = ranksum(squeeze(dens_passive_neg(:,iCluster)),squeeze(dens_rest_neg(:,iCluster)));
   
   if median(squeeze(dens_passive_neg(:,iCluster))) > median(squeeze(dens_rest_neg(:,iCluster))); h_stimulus_neg(iCluster) = -1; end
   if median(squeeze(dens_passive_neg(:,iCluster))) < median(squeeze(dens_rest_neg(:,iCluster))); h_stimulus_neg(iCluster) = 1; end

end

contrast_attention_pos_z = zeros(1,nClusters);
contrast_stimulus_pos_z = zeros(1,nClusters);
contrast_attention_neg_z = zeros(1,nClusters);
contrast_stimulus_neg_z = zeros(1,nClusters);

for iCluster=1:nClusters

    if p_attention_pos(iCluster) < pcriterion; contrast_attention_pos_z(iCluster) = norminv(p_attention_pos(iCluster)); end
    if p_attention_neg(iCluster) < pcriterion; contrast_attention_neg_z(iCluster) = norminv(p_attention_neg(iCluster)); end
    
    if p_stimulus_pos(iCluster) < pcriterion; contrast_stimulus_pos_z(iCluster) = norminv(p_stimulus_pos(iCluster)); end
    if p_stimulus_neg(iCluster) < pcriterion; contrast_stimulus_neg_z(iCluster) = norminv(p_stimulus_neg(iCluster)); end
   
    contrast_attention_pos_z(iCluster) = h_attention_pos(iCluster) * contrast_attention_pos_z(iCluster);
    contrast_attention_neg_z(iCluster) = h_attention_neg(iCluster) * contrast_attention_neg_z(iCluster);
  
    contrast_stimulus_pos_z(iCluster) = h_stimulus_pos(iCluster) * contrast_stimulus_pos_z(iCluster);
    contrast_stimulus_neg_z(iCluster) = h_stimulus_neg(iCluster) * contrast_stimulus_neg_z(iCluster);
    
end

save(strcat('FC-Voxels-',parcellation_label,'-AAL-ROI-corr-KMeans-FC-All-Clusters-Contrast-POSNEG-corrected.mat'),'contrast_stimulus_pos_z','contrast_attention_pos_z','contrast_stimulus_neg_z','contrast_attention_neg_z');

end

function save3DVolumeContrastFCOnAllClustersPOSNEGV2

load('FC-Voxels-MD758-AAL-ROI-corr-KMeans-FC-All-Clusters-Contrast-POSNEG-corrected.mat');
load('FC-Voxels-AAL-ROI-corr-KMeans-Info.mat');
...
contrast_FC_Attention_POS = zeros(MNI_size);
contrast_FC_Attention_NEG = zeros(MNI_size);
contrast_FC_Stimulus_POS = zeros(MNI_size);
contrast_FC_Stimulus_NEG = zeros(MNI_size);
...
for iROI=1:nROIs
    ...
    for iCluster=1:nClusters
        ...
        for iVoxel=1:nVoxels
            ...
            contrast_FC_Attention_POS(idxx,idxy,idxz) = contrast_attention_pos_z(iiCluster);
            contrast_FC_Attention_NEG(idxx,idxy,idxz) = contrast_attention_neg_z(iiCluster);
            contrast_FC_Stimulus_POS(idxx,idxy,idxz) = contrast_stimulus_pos_z(iiCluster);
            contrast_FC_Stimulus_NEG(idxx,idxy,idxz) = contrast_stimulus_neg_z(iiCluster);
...
descrip = 'Contrast-FC-Attention-POS';
fname = strcat('MD758-Contrast-FC-Attention-POS','.nii');
input_data = contrast_FC_Attention_POS; 
real_save_image;

descrip = 'Contrast-FC-Attention-NEG';
fname = strcat('MD758-Contrast-FC-Attention-NEG','.nii');
input_data = contrast_FC_Attention_NEG; 
real_save_image;

descrip = 'Contrast-FC-Stimulus-POS';
fname = strcat('MD758-Contrast-FC-Stimulus-POS','.nii');
input_data = contrast_FC_Stimulus_POS; 
real_save_image;

descrip = 'Contrast-FC-Stimulus-NEG';
fname = strcat('MD758-Contrast-FC-Stimulus-NEG','.nii');
input_data = contrast_FC_Stimulus_NEG; 
real_save_image;


end

function plotContrastFCOnAllClustersV2

% parcellation_label = 'MD758';
% load('FC-Voxels-MD758-AAL-ROI-corr-KMeans-FC-All-Clusters-Contrast-corrected.mat');
% load('FC-Voxels-AAL-ROI-corr-KMeans-Info.mat');

% parcellation_label = 'SP758';
% load('FC-Voxels-SP758-AAL-ROI-corr-KMeans-FC-All-Clusters-Contrast-corrected.mat');
% load('FC-Voxels-AAL-ROI-corr-KMeans-Info.mat');

parcellation_label = 'MD758-NoWavelet';
load('FC-Voxels-MD758-NoWavelet-AAL-ROI-corr-KMeans-FC-All-Clusters-corrected.mat');
load('FC-Voxels-AAL-ROI-corr-KMeans-Info.mat');
...
for imatrix=1:2
    ...
    print(f,'-depsc',strcat('FC-',parcellation_label,'-All-Clusters-Contrast-corrected','-',label,'.eps'));

    mu_z = contrast_attention_z;
    mu_z(isinf(mu_z)) = NaN;
    kk = find( mu_z > z_threshold);
    mu_z_contributing = mu_z(kk);
    rho_contributing = tanh( mu_z_contributing);
    IM_contribution = -0.5 * log2( 1 - rho_contributing.^2 );
    IM_total = nansum( IM_contribution(:) );
    [parcellation_label ' - ATTENTION: Total information content ' num2str( IM_total/length(IM_contribution(:)) ) ' kBytes']
    
    mu_z = contrast_stimulus_z;
    mu_z(isinf(mu_z)) = NaN;
    kk = find( mu_z > z_threshold);
    mu_z_contributing = mu_z(kk);
    rho_contributing = tanh( mu_z_contributing);
    IM_contribution = -0.5 * log2( 1 - rho_contributing.^2 );
    IM_total = nansum( IM_contribution(:) );
    [parcellation_label ' - STIMULUS: Total information content ' num2str( IM_total/length(IM_contribution(:)) ) ' kBytes']
...
function computeGrangerAmongClustersV2
...
% parcellation_label = 'SP758';
% load('FC-Voxels-SP758-AAL-ROI-corr-KMeans-Info-Mean-TS-corrected.mat');

% parcellation_label = 'MD758-NoWavelet';
% load('FC-Voxels-MD758-AAL-ROI-corr-KMeans-Info-Mean-TS-corrected-NoWavelet.mat');

parcellation_label = 'MD758-NoLR';
load('FC-Voxels-MD758-AAL-ROI-corr-KMeans-Info-Mean-TS-corrected-NoLR.mat');
...
for iRun=1:nRuns
    ...
    save(strcat('FC_Voxel_',parcellation_label,'_AAL_ROI_kmeans_Granger_Clusters','-','Run','-',int2str(iRun),'.mat'),'track_GF','track_Gpval','passive_GF','passive_Gpval','rest_GF','rest_Gpval');
...
function computeGrangerAtGroupLevelV2

% parcellation_method = 'MD758';
% parcellation_method = 'SP758';
% parcellation_method = 'MD758-NoWavelet';
parcellation_method = 'MD758-NoLR';
...
for iRun=1:nRuns
    ...
    load(strcat('FC_Voxel_',parcellation_method,'_AAL_ROI_kmeans_Granger_Clusters','-','Run','-',int2str(iRun),'.mat'));
...
save(strcat('FC_Voxel_',parcellation_method,'_AAL_ROI_kmeans_Granger_Clusters','-','Mean-Contrast-corrected','.mat'),'Attention_Contrast','Stimulus_Contrast','track_mean','passive_mean','rest_mean','tracking','passive','rest');
...
load(strcat('FC_Voxel_',parcellation_method,'_AAL_ROI_kmeans_Granger_Clusters','-','Mean-Contrast','.mat'));
load('FC-Voxels-AAL-ROI-corr-KMeans-Info.mat');
...
for iCondition=1:3
    ...
    print(f,'-depsc',strcat('FC_Voxel_',parcellation_method,'_AAL_ROI_kmeans_Granger_Clusters','-','Mean-Contrast-',label,'.eps'));
...
function plotFCAndGrangerAtGroupLevelContrastJBV2(parcellation_label,do)

parcellation_label = 'MD758';
% parcellation_label = 'SP758';

load(strcat('FC-Voxels-',parcellation_label,'-AAL-ROI-corr-KMeans-FC-All-Clusters-Contrast-corrected.mat'));
% load(strcat('FC_Voxel_',parcellation_label,'_AAL_ROI_kmeans_Granger_Clusters','-','Mean-Contrast-corrected','.mat'));
% load(strcat('FC_Voxel_',parcellation_label,'_AAL_ROI_kmeans_Granger_Clusters','-','Mean-Contrast','.mat'));
load('FC-Voxels-AAL-ROI-corr-KMeans-Info.mat');
...
for iCondition=1:3
    ...
    Attention_rho(find(Attention_rho>(-1)*zcriterion & Attention_rho<zcriterion)) = NaN; 
        
    Stimulus_rho = stimulus_z;
    Stimulus_rho(find(Stimulus_rho>(-1)*zcriterion & Stimulus_rho<zcriterion)) = NaN; 
        
    if iCondition == 1; rho = Attention_rho; label = 'Attention'; end
    if iCondition == 2; rho = Stimulus_rho; label = 'Stimulus'; end
    
    disp(strcat(label,':POS:',int2str(length(find(rho>0))),':NEG:',int2str(length(find(rho<0)))));
    
    if iCondition == 3
        
        rho = NaN(size(attention_z));
        rho(find(~isnan(Attention_rho) & ~isnan(Stimulus_rho))) = 3;
    ...
    label = strcat('Contrast','-',do,'-',parcellation_label,'-',label,'-','checker');

    plotFCcheckerboard( rho, [], cmin, cmax, [], 0, 2, label );
...
function plotGrangerAtGroupLevelContrastJBAllLobesV2(parcellation_label)
...
for iRun=1:nRun
    ...
    all_corr_track(iRun,:,:) = tracking(iRun).zscore;
    all_corr_passive(iRun,:,:) = passive(iRun).zscore;
    all_corr_rest(iRun,:,:) = rest(iRun).zscore;
    
end

my_std_track_z = squeeze(std(all_corr_track,0,1));
my_mean_track_z = squeeze(mean(all_corr_track,1));

my_std_passive_z = squeeze(std(all_corr_passive,0,1));
my_mean_passive_z = squeeze(mean(all_corr_passive,1));

my_std_rest_z = squeeze(std(all_corr_rest,0,1));
my_mean_rest_z = squeeze(mean(all_corr_rest,1));

new_label = strcat('GC-',parcellation_label,'-Track-Fisher-checker');
% plotGCgeneral(my_mean_track_z,my_std_track_z,new_label,plotParcelsLabels_idx,ROI);
plotFCcheckerboard( my_mean_track_z, my_std_track_z,cmin, cmax, z_threshold, applyThreshold, applyColorBar, new_label );

new_label = strcat('GC-',parcellation_label,'-Passive-Fisher-checker');
% plotGCgeneral(my_mean_passive_z,my_std_passive_z,new_label,plotParcelsLabels_idx,ROI);
plotFCcheckerboard( my_mean_passive_z, my_std_passive_z, cmin, cmax, z_threshold, applyThreshold, applyColorBar, new_label );

new_label = strcat('GC-',parcellation_label,'-Rest-Fisher-checker');
% plotGCgeneral(my_mean_rest_z,my_std_rest_z,new_label,plotParcelsLabels_idx,ROI);
plotFCcheckerboard( my_mean_rest_z, my_std_rest_z, cmin, cmax, z_threshold, applyThreshold, applyColorBar, new_label );
...
function getMeaningfulClustersWithChangeBasedOnFCV2
...
save('FC-Voxels-AAL-ROI-corr-KMeans-MeaningFul-FC-corrected.mat','MeaningFul');
...
function getMeaningfulClustersWithChangeBasedOnFCGCAttentionStimulusOnlyV2
...
%         all_idx_pos = [all_idx_pos, idx_pos];
    ...
    save(strcat('FC-Voxels-',parcellation_label,'-AAL-ROI-corr-KMeans-MeaningFul-FC-Att-Stim-Only-',change_label,'-corrected.mat'),'MeaningFul_Attention','MeaningFul_Stimulus');
