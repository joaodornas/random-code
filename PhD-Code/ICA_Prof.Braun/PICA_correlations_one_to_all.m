...
%load('Low-High-Jan-08-05-2015-groupICA-Four-lobes-All-Runs-AAL-residual-Parietal.mat');
load('Low-High-Elena-29-05-2015-groupICA-Four-lobes-All-Runs-AAL-residual-Parietal.mat');
...
for nPAIR = 13 : 2 : 14;
    ...
    for nROI = nPAIR : nPAIR + 1;
        ...
        for nRUN = 1 : 2;
            ...
            disp(strcat('SSQ-High-Run-',int2str(nRUN),'-OxO:',num2str(sumsqr(rho_att(1:Ncomponent,1:Ncomponent)))));
            disp(strcat('SSQ-Rest-Run-',int2str(nRUN),'-OxO:',num2str(sumsqr(rho_rest(1:Ncomponent,1:Ncomponent)))));
            
            disp(strcat('SSQ-High-Run-',int2str(nRUN),'-Ox',char(FC_Parietal(1).ROI_Label( nROI )),':',num2str(sumsqr(rho_att(1:Ncomponent,Ncomponent+1:2*Ncomponent)))));
            disp(strcat('SSQ-Rest-Run-',int2str(nRUN),'-Ox',char(FC_Parietal(1).ROI_Label( nROI )),':',num2str(sumsqr(rho_rest(1:Ncomponent,Ncomponent+1:2*Ncomponent)))));
...
hold on;
plot([0 1], [0 0], 'k');
plot( [1:N2]/N2, ZD1-ZD2, 'b', 'LineWidth', 2 );
hold off;
axis('square');
axis([0 1 -0.5 0.5]);
xlabel('fraction');
...
hold on;
plot([0 1], [0 0], 'k');
plot( [1:N2]/N2, ZD1-ZD2, 'r', 'LineWidth', 2 );
hold off;
axis('square');
axis([0 1 -0.5 0.5]);
xlabel('fraction');
