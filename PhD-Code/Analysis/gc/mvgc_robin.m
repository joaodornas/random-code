...
%% Getting Robin's Data

Robin = load('RobinTimeSeries.mat');

T = 25000;

for i=1:100

    X(1,:,i) = Robin.X1dt(((i-1)*T + 1):i*T);
    X(2,:,i) = Robin.Y1dt(((i-1)*T + 1):i*T);
    X(3,:,i) = Robin.X2dt(((i-1)*T + 1):i*T);
    X(4,:,i) = Robin.Y2dt(((i-1)*T + 1):i*T);
...
figure(2); clf;
subplot(1,3,1);
plot_pw(F,colormap('jet'));
title('Pairwise-conditional GC');
subplot(1,3,2);
plot_pw(pval,colormap('jet'));
title('p-values');
subplot(1,3,3);
plot_pw(sig,colormap('jet'));
