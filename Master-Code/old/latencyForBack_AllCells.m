...
function latencyForBack_AllCells
...
for iCell=1:nCells
    ...
    for iRegistro=1:nRegistro
        ...
        save(strcat('ForBack-Cell','-',int2str(iCell),'-','Video','-',int2str(cells_data_file(iCell).registro_video(iRegistro).video),'-',cells_data_file(iCell).registro_video(iRegistro).datafile,'-','latency','.mat'),'latency_time');
           
    end

end

save('ForBack-latency-distribution.mat','latency_distribution');
