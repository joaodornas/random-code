function genCCMetricSpaceFiles
...
for iCell=1:length(cells_data_file)
    ...
    for iVideo=1:nVideos
        ...
        latency_file = strcat('CC-Cell','-',int2str(iCell),'-','Video','-',int2str(cells_data_file(iCell).registro_video(iVideo).video),'-',cells_data_file(iCell).registro_video(iVideo).datafile,'-','latency','.mat');
