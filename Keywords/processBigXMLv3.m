function processBigXMLv3(xmlfile)
...
statFileName = strcat(xmlfile(1:end-4),'-PubMed.mat');
...
function [abstract, fileline] = getAbstract(pubmedfile,fileline,adj,adv,verb,noun,someWordsToRemove)
    ...
    if isempty(idx_spaces)
        ...
        idx_adj = find(strcmpi(abstract,adj));
        idx_adv = find(strcmpi(abstract,adv));
        idx_verb = find(strcmpi(abstract,verb));
        idx_some = find(strcmpi(abstract,someWordsToRemove));
        ...
        for iWords=1:nWordsInAbstract
            ...
            idx_adj = find(strcmpi(abstract{iWords},adj));
            idx_adv = find(strcmpi(abstract{iWords},adv));
            idx_verb = find(strcmpi(abstract{iWords},verb));
            idx_some = find(strcmpi(abstract{iWords},someWordsToRemove));
...
function [articletitlekeywords, fileline] = getArticleTitle(pubmedfile,fileline,start_section,end_section,adj,adv,verb,noun,someWordsToRemove)
    ...
    if isempty(idx_spaces)
        ...
        idx_adj = find(strcmpi(articletitlekeywords,adj));
        idx_adv = find(strcmpi(articletitlekeywords,adv));
        idx_verb = find(strcmpi(articletitlekeywords,verb));
        idx_some = find(strcmpi(articletitlekeywords,someWordsToRemove));
        ...
        for iWords=1:nWordsInArticleTitle
            ...
            idx_adj = find(strcmpi(articletitlekeywords{iWords},adj));
            idx_adv = find(strcmpi(articletitlekeywords{iWords},adv));
            idx_verb = find(strcmpi(articletitlekeywords{iWords},verb));
            idx_some = find(strcmpi(articletitlekeywords{iWords},someWordsToRemove));
