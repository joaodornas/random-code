function processBigXMLv10(xmlfile)
...
wordNet = load('common_verbs.mat');
common_verbs = lower(strtrim(wordNet.common_verbs));
% common_verbs = char.empty;
            ...
            if ~isempty(idx_string_found)
                ...
                switch idx_string_found
                    ...
                    case 5 %% KeywordList
                        ...
                        [processedKeywords, Keywords, line] = getKeywords(xml,line,adj,adv,verb,noun,plnoun,common_verbs);
                    ...
                    case 6 %% Abstract
                        ...
                        if isempty(idx_abstracttext_string)
                            ...
                            [Abstract, line] = getAbstract(xml,line,adj,adv,verb,noun,plnoun,common_verbs);
                    ...
                    case 9 %% ArticleTitle
                        ...
                        [ArticleTitle, line] = getArticleTitle(xml,line,'<ArticleTitle','</ArticleTitle',adj,adv,verb,noun,plnoun,common_verbs);
                    ...
                    case 10 %% CollectionTitle
                        ...
                        [ArticleTitle, line] = getArticleTitle(xml,line,'<CollectionTitle','<CollectionTitle',adj,adv,verb,noun,plnoun,common_verbs);
...
statFileName = strcat(xmlfile(1:end-4),'-PubMed-v10.mat');
...
function [newkeywords, keywords, fileline] = getKeywords(pubmedfile,fileline,adj,adv,verb,noun,plnoun,common_verbs)
    ...
    newkeywords = processParagraph(allkeywords,adj,adv,verb,noun,plnoun,common_verbs);
...
function [abstract, fileline] = getAbstract(pubmedfile,fileline,adj,adv,verb,noun,plnoun,common_verbs)
    ...
    abstract = processParagraph(allAbstract,adj,adv,verb,noun,plnoun,common_verbs);
...
function [authors, fileline] = getAuthors(pubmedfile,fileline,end_section) 
    ...
    while isempty(idx_end_of_section_authorlist)
        ...
        if ~isempty(idx_start_of_author_one) || ~isempty(idx_start_of_author_two) || ~isempty(idx_start_of_lastname)
        ...
        if ~isempty(idx_start_of_lastname)
            ...
            if ~isempty(idx_end_of_lastname); end_ = idx_end_of_lastname; else end_ = length(fileline); end
                
            authors{iAuthors,1} = fileline(idx_start_of_lastname+length(start_of_lastname):end_);
            
        end
        
        if ~isempty(idx_start_of_forename)
            
            if ~isempty(idx_end_of_forename); end_ = idx_end_of_forename-1; else end_ = length(fileline); end
        
            authors{iAuthors,2} = fileline(idx_start_of_forename+length(start_of_forename):end_);
...
function [articletitlekeywords, fileline] = getArticleTitle(pubmedfile,fileline,start_section,end_section,adj,adv,verb,noun,plnoun,common_verbs)
    ...
    articletitlekeywords = processParagraph(articletitle,adj,adv,verb,noun,plnoun,common_verbs);
...
function allkeywords = processParagraph(input_string,adj,adv,verb,noun,plnoun,common_verbs)
...
%tic    
    if isempty(idx_spaces)
        ...
        if ~isempty(allkeywords)
            ...
            idx_common_verbs = strmatch(allkeywords,common_verbs);

%             idx_adj = strmatch(allkeywords,adj);
%             idx_adv = strmatch(allkeywords,adv);
%             idx_verb = strmatch(allkeywords,verb);
            
            if ~isempty(idx_adj) || ~isempty(idx_adv) || ~isempty(idx_verb) || ~isempty(idx_common_verbs)
...
%              tic
            ...
            idx_common_verbs = strmatch(allkeywords{iWords},common_verbs);

%             idx_adj = strmatch(allkeywords{iWords},adj);
%             idx_adv = strmatch(allkeywords{iWords},adv);
%             idx_verb = strmatch(allkeywords{iWords},verb);
