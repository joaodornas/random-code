function processBigXMLv7(xmlfile)
...
statFileName = strcat(xmlfile(1:end-4),'-PubMed-v7.mat');
...
function [abstract, fileline] = getAbstract(pubmedfile,fileline,adj,adv,verb,noun,plnoun)
    ...
    while isempty(idx_end_of_section_abstract)
        ...
        if ~isempty(idx_start_of_sub_section_abstracttext); foundStartSubSection = true; end
        
        end_of_sub_section_abstracttext = '</AbstractText>';
        if foundStartSubSection; idx_end_of_sub_section_abstracttext = strfind(fileline,end_of_sub_section_abstracttext); end
...
function allkeywords = processParagraph(input_string,adj,adv,verb,noun,plnoun)
    ...
    if isempty(idx_spaces)
        ...
        allkeywords = strsplit(input_string);
        
        idx_spaces = isspace(allkeywords);
    ...
    if isempty(idx_spaces)
        ...
        if ~isempty(allkeywords)
            ...
            idx_adj = find(strcmpi(allkeywords,adj));
            idx_adv = find(strcmpi(allkeywords,adv));
            idx_verb = find(strcmpi(allkeywords,verb));
        ...
        for iWords=1:nWordsInArticleTitle
            ...
            idx_adj = find(strcmpi(allkeywords{iWords},adj));
            idx_adv = find(strcmpi(allkeywords{iWords},adv));
            idx_verb = find(strcmpi(allkeywords{iWords},verb));
    ...
    if isempty(idx_spaces)
        ...
        if ~isempty(allkeywords)
            ...
            check_if_is_plural = [];
            check_if_is_plural = strmatch(allkeywords,plnoun);
        ...
        for iWords=1:nWordsInArticleTitle
            ...
            check_if_is_plural = [];
            check_if_is_plural = strmatch(allkeywords{iWords},plnoun);
