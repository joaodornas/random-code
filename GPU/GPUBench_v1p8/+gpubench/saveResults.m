...
% If we are adding host data, clear any existing host data in the file - we
% are only allowed one!
if newResult.IsHostData
    results([results.IsHostData]) = [];
else
    newName = newResult.GPUInfo.Name;
    % Check to see if it already exists
    deviceNames = cell(N,1);
    for ii=1:N
        if results(ii).IsHostData
            deviceNames{ii} = results(ii).CPUInfo.Name;    
        else
            deviceNames{ii} = results(ii).GPUInfo.Name;
        end
    end
    toRemove = strcmp( deviceNames, newName );
    results( toRemove ) = [];
end
