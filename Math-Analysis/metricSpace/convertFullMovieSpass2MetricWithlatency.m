function X = convertFullMovieSpass2MetricWithlatency(date,site_index,channel,registro,video_index,start_time,end_time,inverter_movie,bin_size,nRepetitions,ae)
...
switch vn
    ...
    case 1
        ...
        category{1} = 'Video 1: Swans. DIRECT With Latency';
        category{2} = 'Video 1: Swans. REVERSE With Latency';
        category{3} = 'Video 1: Spontaneous Activity';
                
    case 2
        
        category{1} = 'Video 2: Mountains. DIRECT With Latency';
        category{2} = 'Video 2: Mountains. REVERSE With Latency';
        category{3} = 'Video 2: Spontaneous Activity';
        
    case 3
        
        category{1} = 'Video 3: Lizard. DIRECT With Latency';
        category{2} = 'Video 3: Lizard. REVERSE With Latency';
        category{3} = 'Video 3: Spontaneous Activity';
        
    case 4
        
        category{1} = 'Video 4: Beetle. DIRECT With Latency';
        category{2} = 'Video 4: Beetle. REVERSE With Latency';
...
filepath = strcat('/Volumes/Data/DATA/Forward-Backward/Jerome/metricMovieWithlatency/',int2str(start_time),'-',int2str(end_time),'/',registro,'-','v',int2str(video_index));

if ae == 1
        
        mkdir(strcat(filepath,'/','full-movie/full-movie-ae'));
        folder = 'full-movie/full-movie-ae';
        file_type = 'full-movie-ae-With-latency';
    ...
    if (inverter_movie == 1)
        ...
        file_type = strcat('bin-size-',int2str(bin_size),'-','full-movie-invertido-With-latency');
        ...
        file_type = 'full-movie-original-With-latency';
...
for i=1:nConditions
    ...
    if ae == 1
        ...
        if i ~= 2
            ...
            my_category = getCategory(trials_spikes,start_time,end_time,i,N,bin_size,category,inverter_movie);

            categories(w) = my_category;
...
function the_category = getCategory(trials_spikes,start_time,end_time,i,N,bin_size,category_name,inverter_movie)
