load('SF_tuningDynamics_analysis_windowSize10_anaPeriod1000','sigmas_dyn','LSFSs_dyn','HSFSs_dyn','pref_SFs_dyn_octv', ...
    'delta_sf','delta_sigma','delta_lsfs','delta_hsfs','sign_index_sf','sign_index_sigma','sign_index_lsfs','sign_index_hsfs');
...
for i=1:1000
    ...
    larger=find(pref_SF_boot(i,1:100)>pref_SF_boot(i,1));
    smaller=find(pref_SF_boot(i,1:100)<pref_SF_boot(i,1));
    sign_index_sf_boot(i)=(numel(larger)-numel(smaller))/(numel(larger)+numel(smaller));
end

delta_sf_boot=nanmean(pref_SF_boot(:,901:1000),2)-nanmean(pref_SF_boot(:,1:10),2);
...
p_deltaSF=ranksum(delta_sf(find(~isnan(delta_sf))),delta_sf_boot(find(~isnan(delta_sf_boot))))
...
p_signIndSF=ranksum(sign_index_sf(find(~isnan(sign_index_sf))),sign_index_sf_boot(find(~isnan(sign_index_sf_boot))))
...
[r_SFdyn,p_SFdyn]=corr(mean_pref_SF',mean_pref_SF_boot','type','spearman','rows','pairwise')
...
r_SFdyn_iterations=nan(1000,1); p_SFdyn_iterations=nan(1000,1);
...
for i=1:1000
    [r_SFdyn_iterations(i),p_SFdyn_iterations(i)]=corr(mean_pref_SF',pref_SF_boot(i,:)','type','spearman','rows','pairwise');
...
mean_r_iterations_sf=nanmean(r_SFdyn_iterations)
mean_p_iterations_sf=nanmean(p_SFdyn_iterations)
nSiginif_iterations_sf=numel(find(p_SFdyn_iterations<0.05))
