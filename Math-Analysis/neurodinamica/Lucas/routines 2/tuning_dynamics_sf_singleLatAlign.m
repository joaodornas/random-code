% load ('latForEachFreq','complex_cells','SFs_i_atBestTF','SFvalues_octave','lat_SFs_atBestTF','SF_columnLabels_octv','latency_allCells_maxLikelihood','i_bestCondition');
% ncells=numel(complex_cells);
% analysis_period=200;
% analysis_startTime=1;
% window_size=10;
...
for i=41:92%[1:4,6:12,14:ncells]
    ...
    lat=latency_allCells_maxLikelihood(i_bestCondition(i));
    if isnan(lat)
        lat=48;
    end
    sf_responses=zeros(numel(SFs_i),1);
    sf_responses_cell=nan(numel(SFvalues_octv),analysis_period);
    %sf_response_sem=zeros(numel(SFs_i),1);
    ...
    for k=analysis_startTime:analysis_period+analysis_startTime-1 %slide windows, fit and extract parameters
        ...
        sf_responses=mean(SF_psths(:,baseline_duration+k+lat-(window_size/2):baseline_duration+k+lat+(window_size/2)),2);
        %sf_response_sem(:,1)=std(SF_psths(baseline_duration+k+lat-(window_size/2):baseline_duration+k+lat+(window_size/2)),2)/sqrt(window_size);
...
save(['/Users/lucaspinto/Documents/Lab/ProjectBooks/CV&Dyn-Book/Analyses/tuningDynamics/SF_tuningDynamics_windowSize' num2str(window_size) '_' num2str(analysis_period) '_singleLatAlign.mat']);
