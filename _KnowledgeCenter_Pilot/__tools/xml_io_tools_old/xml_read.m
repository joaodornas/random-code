  ...
  if (LeafNode>0) % supported leaf node types
    try
      try         % use try-catch: errors here are often due to VERY large fields (like images) that overflow java memory
        ...
        % convert to number(s) if needed %%% DORNAS Update: Do not change
        % to number !!
...
if (node.hasAttributes && Pref.ReadAttr)
  ...
  for iAttr = 1:Attr.getLength    % for each attribute
    ...
    %%% DORNAS Update: check if it is numeric before changing to char
    value = Attr.item(iAttr-1).getValue;
    idx = strfind(value,'-');
    if ~isempty(idx); value = strcat(value(1:idx-1),'-',value(idx+1:end)); end
    value = char(value); % attribute value 
    
    % value = str2var(value, Pref.Str2Num, 1);   % convert to number if
    % possible %%% DORNAS Update: I do not want to convert to number !!
