...
full_path = 'D:\__PUBMED-references\utf-8-repeat\'; 
list = dir(strcat(full_path,which_query,'*.xml'));
        
nFiles = length(list);
       
       for iFile=1:nFiles
       
       fileID = fopen(strcat('log','-',query_label,'-',int2str(query_id),'-',int2str(start_year),'-',int2str(end_year),'.txt'),'a+');
           ...
           if ~isempty(xml_tree) && isstruct(xml_tree)
              ...
              if ~strcmp('ERROR',apriori_fields)
                  ...
                  if nPublications > 0
                      ...
                      if I_Am_Testing
                        conn = database('references_papers_tmp','joaodornas','Senh@1111','Vendor','MySQL','Server','141.44.42.20'); %%% TEST DATABASE
                      else
                      ...
                      for iPub=1:nPublications
                          ...
                          clearvars -except iPub iYear nPublications xml_tree apriori_fields iFile nFiles list full_path_year start_year end_year full_path conn query_id query_label iPub_data I_Am_Testing fileID OS
